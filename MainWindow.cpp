...
MainWindow::MainWindow(QWidget *parent)
  ...
  MenuFile->addActions(
      QList<QAction *>({{this->ActionNew, this->ActionOpen, this->ActionSave,
                         this->ActionSaveAs}}));
  ...
  MenuDocumentation->addActions(QList<QAction *>(
      {{this->ActionDocSetPath, this->ActionAddDocument,
        this->ActionRemoveOrphean, this->ActionUpdateDocuments}}));
 ...
 connect(this->ActionOpen, &QAction::triggered,
          [this]() { actionOpenTriggered(); });
  connect(this->ActionSave, &QAction::triggered,
          [this]() { actionSaveTriggered(); });
  connect(this->ActionExit, &QAction::triggered, [this]() { close(); });

  // Documentation connections
  connect(this->ActionAddDocument, &QAction::triggered,
          [this]() { actionAddDocumentTriggered(); });
...
MainWindow::~MainWindow() {
    // Delete singleton instances here, because QApplication doesn't always return
    // to main()
    Languages::release();
    Settings::release();
    WorkingDB::release();
...
void MainWindow::updateRecentFileMenu() {
  ...
  for (int i = 0; i < rdb.size(); i++) {
    ...
    QAction *action = new QAction(
        filename, this->MenuRecent); // Give parentality to the menu, so
                                     // clear() can delete the action
    this->MenuRecent->addAction(action);
    connect(action, &QAction::triggered,
            [this, filename]() { openRecent(filename); });
  }
}
...
void MainWindow::actionNewTriggered() {
  if (WorkingDB::instance()->newDB(this)) {
    Settings::instance()->addRecentDatabase(
        WorkingDB::instance()->getFilename());
    updateRecentFileMenu();
    updateUI();
  }
}
...
void MainWindow::openRecent(QString filename) { (void)filename; }
